      ...
      <div className="mx-auto flex w-full max-w-(--ds-layout-page-max) flex-col gap-5 px-4 py-5 sm:gap-6 sm:px-6 sm:py-7 lg:px-8 lg:py-8">
        <header className="flex flex-col gap-4 border-b border-(--ds-color-border) pb-4 lg:flex-row lg:items-end lg:justify-between lg:pb-5">
          <div className="flex w-full min-w-0 items-start justify-between gap-3 lg:w-auto lg:max-w-(--ds-layout-readable-max)">
            ...
            {titleActions ? <div className="shrink-0 lg:hidden">{titleActions}</div> : null}
          </div>
          {actions ? (
            <div className="flex w-full flex-wrap items-center gap-2 lg:w-auto">
              {titleActions ? <div className="hidden lg:block">{titleActions}</div> : null}
              {actions}
            </div>
          ) : titleActions ? (
            <div className="hidden flex-wrap items-center gap-2 lg:flex lg:w-auto">{titleActions}</div>
          ) : null}
        </header>
